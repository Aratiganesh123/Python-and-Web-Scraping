...
def readMarkSheet(file_name):
	...
	next(reader)
	for line in reader:
		name_marks_mapping[line[0]] = {'marks':[float(line[i]) for i in range(1,len(line))]}
...
def generateGradeCard(mapping_dict):
	...
	for key,value in mapping_dict.items():
		marks = list(value.values())[0]
		grade_card[key] = {'subject_wise_marks':marks}
		total = float(sum(marks)*100/(len(marks)*100))
		if(total >= 90):
			grade = 'O'
		...
		grade_card[key]['grade_received'] = grade
